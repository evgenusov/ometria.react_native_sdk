...
const OmetriaReactNativeSdk = NativeModules.OmetriaReactNativeSdk;
...
OmetriaReactNativeSdk.onNotificationOpenedApp = async remoteMessage => {
  ...
  _onNotificationInteracted(Platform.OS === 'ios' ? iOSRemoteMessage : remoteMessage);
...
OmetriaReactNativeSdk.onNotificationReceived = remoteMessage => {
  ...
  _onNotificationReceived(Platform.OS === 'ios' ? iOSRemoteMessage : remoteMessage);
};

// 🛠️ Custom Implementation: parseNotification()
OmetriaReactNativeSdk.parseNotification = async notification => {
  const parsedNotification = Platform.OS === 'android' ? await _parseNotification(notification) : Promise.resolve(notification?.data?.ometria ? JSON.parse(notification.data.ometria) : undefined);
...
OmetriaReactNativeSdk.onAndroidBackgroundMessage = async ({
  ...
  Platform.OS === 'android' && OmetriaReactNativeSdk.initializeWithApiToken(ometriaToken, ometriaOptions).then(async () => {
    OmetriaReactNativeSdk.onNotificationReceived(remoteMessage);
  });
};
